...
/**
 * 支持的输入模态（pi-ai Model.input，即 DSH 的 inputModalities）。
 * 与 dsh-llm-pi-ai 的 MODALITIES 完全一致：text、image。
 */
const INPUT_MODALITIES = ['text', 'image']

/** 未声明 input 时的默认值：纯文本，保持历史行为不变。 */
...
const modelSchema = z.object({
  ...
  // 可选：声明该模型可接受的输入模态。
  // 缺省 ['text']（纯文本），声明 ['text','image'] 才会被 DSH 认作视觉模型 ——
  // dsh-llm-pi-ai 把 Model.input 原样映射为 inputModalities，而 dsh-tool-fs 的
  // read_image 会拒绝任何未声明 image 的路由（"does not declare image input"）。
  // 注意：这只是「声明」，不代表上游端点真的收图；声明错误只会在真正传图时报错。
  input: z.array(z.union(INPUT_MODALITIES)).default([...DEFAULT_INPUT]),
})
...
function buildModel(providerId, provider, modelId, entry) {
  ...
  return {
    ...
    // 与 dsh-llm-pi-ai 的 declaredInput 语义对齐：缺省与空数组同义（都视为「未声明」），
    // 回落到纯文本，避免 `input: []` 产出一个谁也用不了的退化声明。
    input:
      entry.input === undefined || entry.input.length === 0 ? [...DEFAULT_INPUT] : [...entry.input],
...
function buildProfiles(providers) {
  ...
  for (const [providerId, source] of Object.entries(providers ?? {})) {
    ...
    // 对齐 @deepseek-ai/dsh-llm-pi-ai 0.1.5 的 resolveProfiles 产出：
    // 0.1.5 起 modelOf() 会无条件读取 profile.modelErrors（缺字段抛
    // “Cannot read properties of undefined (reading 'get')”），
    // 模型目录枚举（模型选择器）对每个模型调 modelInfo → 整组 provider 变“加载失败”。
    // 图片预算字段同样由 pi-ai 裸读，补上官方默认值防图片请求路径炸。
