  ...
  factory: (require) => {
    ...
    class ThinkingApiStore {
      ...
      async saveProvider(route, profile, keyValue) {
        ...
        // 0.1.5：settings/mutate(ns, ops, expectedRevision) → { ok, value, ... }。
        const response = await this.ctx.remote.settings.mutate(NS, [{ op: "set", path: ["providers", route], value: profile }]);
        ...
        if (storesKey) {
          ...
          if (!stored.ok) return `Provider saved, but the API key could not be stored (${stored.error.message}). Re-save to retry.`;
        }
        await this.load();
        return undefined;
      }
      ...
      async removeProvider(route) {
        const keyRef = deriveKeyRef(route);
        await this.ctx.remote.credentials.unset(keyRef).catch(() => {});
        const response = await this.ctx.remote.settings.mutate(NS, [{ op: "unset", path: ["providers", route] }]);
        if (!response.ok) return response.error.message;
        await this.load();
        return undefined;
      }
    ...
    function apply(ctx) {
      ...
      }, "thinking-api: pushed invalidations");
